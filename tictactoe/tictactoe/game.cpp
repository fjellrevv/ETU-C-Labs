...
char check(char game_field[14 * 2 + 1][14 * 2 + 1], long x, long i, long j, char z, int a, int b) { // функция проверки выигрыша
    ...
    for (int k = 3; k < 2 * b + 1; k += 2) {
        if (game_field[i][k] != game_field[i][k - 2]) {
            cnt = 1;
        }
        else {
            if (game_field[i][k] == z) cnt++;
            if (cnt == x) {
                win = 1;
                break;
            }
    ...
    for (int k = 3; k < 2 * a + 1; k += 2) {
        if (game_field[k][j] != game_field[k - 2][j]) {
            cnt = 1;
        }
        else {
            if (game_field[k][j] == z) cnt++;
            if (cnt == x) {
                win = 1;
                break;
            }
    ...
    for (int k = i + 2; k < 2 * a + 1; k += 2) {
        if (j + 2 < 2 * b + 1) {
            j += 2;
            if (game_field[k][j] != game_field[k - 2][j - 2]) {
                cnt = 1;
            }
            else {
                if (game_field[k][j] == z) cnt++;
    ...
    while (1) {
        ...
        if (key == 80 || key == 155 || key == 235 || key == 115) { // вниз
            ...
            if (menu == 3 && cnt < n * m) {
                ...
                for (int i = a + 2; i < n * 2 + 1; i += 2) { // ищем пустые ячейки снизу
        ...
        if (key == 77 || key == 130 || key == 162 || key == 100) { // вправо
            ...
            if (menu == 3 && cnt < n * m) {
                ...
                for (int j = b + 2; j < 2 * m + 1; j += 2) { // смотрим элементы справа
                ...
                if (lock) { // справа всё занято
                    ...
                    for (int i = a + 2; i < 2 * n + 1; i += 2) // ищем ячейки снизу слева направо
                    {
                    ...
                    if (pol == 0) {
                        ...
                        for (int i = 1; i < 2 * n + 1; i += 2) // ищем ячейки с начала
                        {
                            for (int j = 1; j < 2 * m + 1; j += 2) {
